...
describe('<ScreenRecipeCreate>', () => {
  ...
  it('should create a recipe when submitted with 2 ingredients', async () => {
    ...
    await act(async () => {
      await userEvent.click(screen.getByTestId('recipe-add-ingredient-1-button'));
      await userEvent.type(screen.getByTestId('recipe-name-input'), 'Gnocci', { delay: 1 });
      await userEvent.type(screen.getByTestId('recipe-description-input'), 'Test');
      await userEvent.type(screen.getByTestId('recipe-ingredient-1-input'), 'Flour');
      await userEvent.type(screen.getByTestId('recipe-ingredient-2-input'), 'Eggs');
      return userEvent.click(screen.getByText('Submit'));
    });
