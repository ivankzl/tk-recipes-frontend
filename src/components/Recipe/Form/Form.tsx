...
function Form({
  ...
  const [ingredients, setIngredients] = useState(
    initialRecipe?.ingredients || [{ name: '' }]
  )
  ...
  const onSubmit: SubmitHandler<RecipeFormValues> = async (data) => {
    ...
    initialId
      ? await updateRecipe(initialId, payload)
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <ErrorMessage>
          {errors.name && (
            <span>This field is required and max length is 200</span>
          )}
        </ErrorMessage>
        <FormGroup>
        ...
        <FormGroup>
          ...
          {ingredients.map((inputField, index) => (
            <div key={`${inputField}~${index}`}>
              ...
              <span>
                <ErrorMessage>
                  {errors.ingredients?.length && errors.ingredients[index] && (
                    <span>This field is required and max length is 200</span>
                  )}
                </ErrorMessage>
